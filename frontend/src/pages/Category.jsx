...
const Category = () => {
  ...
  useEffect(() => {
    axiosUser.get('/loaisach')
      .then(r => setCategories((r.data?.data || r.data || []).map(c => ({
        id: c.id,
        ten_loai: c.ten_loai || c.tenLoai
      }))))
      .catch(() => {});
  }, []);
  ...
  const fetchBooks = useCallback(async () => {
    ...
    try {
      ...
      const r = await axiosUser.get('/sach/filter', { params });
      const pg = r.data.data; // Laravel: res.data.data chứa { data: [], total: ... }
      
      const mappedBooks = (pg.data || []).map(s => ({
        id: s.id,
